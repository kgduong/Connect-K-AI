...
public:
    ...
    int alphaBetaSearchMinValue(int depth, int currentDepth, int alpha, int beta,
                                long int originalTime, bool *valid, map<string, vector<Move> > &depthVsMoves);
    //Actual AB Search
    Move alphaBetaSearch(int maxDepth, map<string, vector<Move> > &depthVsMoves, long int originalTime);
    
    //Iterative Depth Search
    Move IDS();
        
    int getHeuristic();//Returns score for the gameState at the time
    
    //Scanes utility of every possible win for board
    int getRIGHTUtility();
    int getUPUtility();
    int getDOWNRIGHTUtility();
    int getUPRIGHTUtility();
    
    
    bool getTerminalState(Move m);
